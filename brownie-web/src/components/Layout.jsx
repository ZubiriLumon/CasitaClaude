...
function NavIcon({ type, active }) {
  const color = active ? '#5D3A1A' : '#8D6E63'
  const size = 22
  ...
  const icons = {
    home: (
      <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
        ...
        <path d="M6 11 L6 19 Q6 20 7 20 L10 20 L10 16 Q10 15 12 15 Q14 15 14 16 L14 20 L17 20 Q18 20 18 19 L18 11" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" fill={active ? '#FFB06730' : 'none'} />
      </svg>
    ),
    cart: (
      <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
        <path d="M3 3 L5 3 L8 16 L18 16" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
        <path d="M6 6 L20 6 L18 14 L7.5 14" stroke={color} strokeWidth="2" strokeLinejoin="round" fill={active ? '#7ECFB330' : 'none'} />
    ...
    box: (
      <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
        <path d="M3 8 L12 4 L21 8 L21 18 L12 22 L3 18 Z" stroke={color} strokeWidth="2.5" strokeLinejoin="round" fill={active ? '#8B5E3C20' : 'none'} />
    ...
    clipboard: (
      <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
        <rect x="5" y="4" width="14" height="18" rx="2" stroke={color} strokeWidth="2.5" fill={active ? '#FFB06720' : 'none'} />
    ...
    chart: (
      <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
        <rect x="4" y="13" width="4" height="8" rx="1" fill={active ? '#8B5E3C' : color} opacity={active ? 0.8 : 0.5} />
        <rect x="10" y="8" width="4" height="13" rx="1" fill={active ? '#FFB067' : color} opacity={active ? 0.8 : 0.5} />
        <rect x="16" y="4" width="4" height="17" rx="1" fill={active ? '#7ECFB3' : color} opacity={active ? 0.8 : 0.5} />
